# Declaração de Acessibilidade

Musterbetrieb GmbH compromete-se a tornar o sítio web Musterbetrieb acessível, nos
termos do Decreto-Lei n.º 82/2022, de 6 de dezembro, que transpõe para a ordem jurídica
portuguesa a Diretiva (UE) 2019/882 (Ato Europeu da Acessibilidade). As suas obrigações
relativas a produtos e serviços aplicam-se desde 28 de junho de 2025.

A presente declaração de acessibilidade aplica-se a https://example.at.

## Estado de conformidade

Este sítio web está parcialmente conforme com EN 301 549 V3.2.1 (WCAG 2.2 AA). Os conteúdos
indicados abaixo não são acessíveis, pelos motivos apresentados.

## Conteúdos não acessíveis

- Die eingebettete Karte hat keinen Titel.
  Requisito afetado: WCAG 4.1.2, EN 301 549 9.4.1.2
  Motivo: a barreira é conhecida e está a ser corrigida.
  Correção prevista até: 31 de dezembro de 2026
- Ältere PDF-Dokumente sind nicht barrierefrei.
  Motivo: encargo desproporcionado.
- Form field must not have multiple label elements
  Páginas afetadas: index.html
  Motivo: a barreira é conhecida e está a ser corrigida.
  Detetado por um teste automático (axe-core, regra form-field-multiple-labels); descreva-o por palavras suas.
- Images must have alternative text
  Requisito afetado: WCAG 1.1.1, EN 301 549 9.1.1.1
  Páginas afetadas: index.html
  Motivo: a barreira é conhecida e está a ser corrigida.
  Detetado por um teste automático (axe-core, regra image-alt); descreva-o por palavras suas.
- Elements must meet minimum color contrast ratio thresholds
  Requisito afetado: WCAG 1.4.3, EN 301 549 9.1.4.3
  Páginas afetadas: blog/2026-06-eaa.html, blog/index.html, impressum.html, index.html, kontakt.html e mais 2
  Motivo: a barreira é conhecida e está a ser corrigida.
  Detetado por um teste automático (axe-core, regra color-contrast); descreva-o por palavras suas.
- Document should have one main landmark
  Requisito afetado: WCAG 1.3.1, EN 301 549 9.1.3.1
  Páginas afetadas: kontakt.html, team.html
  Motivo: a barreira é conhecida e está a ser corrigida.
  Detetado por um teste automático (axe-core, regra landmark-one-main); descreva-o por palavras suas.

## Elaboração desta declaração

A presente declaração foi elaborada em 21 de agosto de 2026.

Baseia-se numa autoavaliação realizada por Musterbetrieb GmbH.

O teste automático de 21 de agosto de 2026 abrangeu 8 páginas
deste sítio web.
2 outras verificações de regras requerem uma decisão humana.
12 verificações de regras não puderam ser decididas pela ferramenta
utilizada; não são apresentadas como cumpridas.

A avaliação baseia-se em parte em testes automáticos. As ferramentas automáticas detetam
apenas uma parte das barreiras possíveis; não substituem testes manuais nem testes com
tecnologias de apoio.

## Comentários e contactos

Encontrou uma barreira ou precisa de informação num formato acessível? Contacte-nos:

- Correio eletrónico: office@example.at
- Formulário de contacto: https://example.at/kontakt
- Telefone: +43 1 2345678
- Morada: Hauptstraße 1, 1010 Wien

Procuramos responder com a maior brevidade possível.

## Procedimento de reclamação

Se não ficar satisfeito com a nossa resposta, pode apresentar uma reclamação à entidade
fiscalizadora competente. A fiscalização está repartida por várias entidades: para os
serviços de comércio eletrónico é a Autoridade Nacional de Comunicações (ANACOM), e os
serviços bancários, por exemplo, são fiscalizados pelo Banco de Portugal. O Instituto
Nacional para a Reabilitação (INR, I.P.) acompanha a aplicação do regime e encaminha para
a entidade competente as queixas que lhe sejam dirigidas diretamente.

Autoridade Nacional de Comunicações (ANACOM)
https://www.anacom.pt

Os sítios web e as aplicações móveis dos organismos do setor público estão sujeitos a um
regime próprio, que prevê a sua própria declaração de acessibilidade. O presente documento
não a substitui.

---

Esta declaração foi gerada com o eaa-kit e não constitui aconselhamento jurídico. Reveja-a
antes de a publicar e, em caso de dúvida, peça a um jurista que a verifique.
